...
import { FbDispatch, FbThunk } from "@/types/redux.types";
...
export const thunkDispatchFileAction =
  (data: FileActionData<FileAction>): FbThunk =>
  (_dispatch, getState) => {
 ...
 * actions. When action is requested, Fb "prepares" the action data by extracting it
 * from Redux state. Once action data is ready, Fb executes some side effect and/or
