...
import { ActionGroup, FileActionMenuItem } from "@/types/action-menus.types";
...
const reducers = {
  ...
  setFileActionGroup(
    state: RootState,
    action: PayloadAction<Record<string, ActionGroup>>,
  ) {
    state.fileActionGroups = action.payload;
  },
  ...
  setCutState(
    state: RootState,
    action: PayloadAction<{ files: FileArray; source?: FileData }>,
  ) {
  ...
  setSelectionMode(state: RootState, action: PayloadAction<boolean>) {
